...
_CONFIG_PATH = Path(__file__).parent.parent / "rumi_mcp_servers.json"


def load_rumi_mcp_configs() -> dict:
 """Return mcp_servers dict from rumi_mcp_servers.json with env vars resolved.


 CWD is injected at load time so ${CWD} in the config always resolves to the
 directory where rumi claude was launched — not the package install location.
 """
