...
from rumi_claude.skills.registry import SkillRegistry, SkillNotFoundError
...
def get_registry() -> SkillRegistry:
   ...
   skills_dir resolution order:
     1. Read "skills.skills_dir" from config.yaml  →  e.g. ".rumi/skills"
     2. Resolve relative to Path.cwd()             →  the target project root
        (rumi_claude is invoked from the project being analysed, so cwd()
         IS the target project, not the rumi package itself)

   Example:
     User runs rumi_claude from /home/user/my_project/
     config.yaml says skills_dir: .rumi/skills
     → skills are loaded from /home/user/my_project/.rumi/skills/
   """
   global _registry
   if _registry is None:
       skills_dir = Path.cwd() / config.get("skills", {}).get("skills_dir", ".rumi/skills")
